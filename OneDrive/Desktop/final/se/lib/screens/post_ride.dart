...
class _PostRideState extends State<PostRide> {
  ...
  Future<void> _postRide() async {
    ...
    if (aadhaarFile == null || driverPhoto == null || vehicleRegC.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Please fill all driver and vehicle details.')),
      );
      return;
    }
              ...
              Column(
                ...
                children: [
                  GooglePlaceAutoCompleteTextField(
                    textEditingController: fromC,
                    googleAPIKey: googleApiKey,
                    inputDecoration: const InputDecoration(
                      labelText: 'Pickup Location (type or select)',
                    ...
                    getPlaceDetailWithLatLng: (prediction) {
                      _setLatLngFromPrediction(prediction, true);
                    },
                  ),
                  const SizedBox(height: 10),
                  GooglePlaceAutoCompleteTextField(
                    textEditingController: toC,
                    googleAPIKey: googleApiKey,
                    inputDecoration: const InputDecoration(
                      labelText: 'Drop Location (type or select)',
                      border: OutlineInputBorder(),
                      prefixIcon: Icon(Icons.location_on),
                    ),
                    debounceTime: 400,
                    ...
                    isLatLngRequired: true,
                    getPlaceDetailWithLatLng: (prediction) {
                      _setLatLngFromPrediction(prediction, false);
                    },
                  ),
                ],
              ),
